 ...
 * stimulus.
 *
 * Edit, 10/21/2016 (cjew)
 * The function showStimImages has been edited to accept stimulus position order as a
 * parameter for each individual trial
 *
 */
...
VisworldBlock.prototype = {
    ...
    showStimImages: function() {
        ...
        if (this.randomizeImagePositions == true) {
        	...
        	console.log(this.stims.side[this.n])
            //positions = this.imagePositions; // uses the same ordering for all trials
            if (this.stims.side[this.n] == 'left') {
            	positions = ['left', 'right'];
            } else {
            	positions = ['right', 'left'];
            }
            //positions = this.stimuli.stimPosition
        }
    ...
    endFamiliarize: function() {
        ...
        $("#instructions")
            .html('<h3>Start of experiment</h3>'+
                  ...
                  '<p>  There are {0} trials.  This part of the experiment should take about {2} minutes or less.  The progress bar at the top will show you how many trials you have completed and how many trials remain.</p>'.format(numTrials, this.breakEvery, timeNearestFiveMins))
            .show();
