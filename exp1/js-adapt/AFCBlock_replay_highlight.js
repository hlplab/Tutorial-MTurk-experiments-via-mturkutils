...
function AFCBlock_replay(params) {
    // process parameters
    ...
    for (p in params) {
        ...
        case 'respKeys':
            ...
            break;
        case 'probes1':
            this.probes1 = params[p];
            break;
        case 'probes2':
            this.probes2 = params[p];
            break;
        case 'targets':
            this.targets = params[p];
            break;
        case 'categories':
...
//        $(this.auds).bind('ended.' + this.namespace, function() {_self.waitForResp();});
                    ...
                    $("#testInstructions").show().addClass("dimmed");        
                     if (_self.mediaType == 'audio') 
                     {
                         ...
                         $originalAudio.on('ended', function() 
                         {
                            ...
                            $("#subtitle1").show().text(_self.stimuliObj.probes1[_self.stims[_self.n]]).css({"color":"", "font-weight":""});
                            $("#subtitle2").show().text(_self.stimuliObj.probes2[_self.stims[_self.n]]).css({"color":"", "font-weight":""});
                            console.log("The word on the left is " + _self.stimuliObj.probes1[_self.stims[_self.n]])
    ...
    fb: function(e) {
                ...
                if((this.respKeys[String.fromCharCode(e.which)] === this.stimuliObj.correctKeys[this.stims[this.n]]))
                        {
                        console.log('Fast, showing feedback!');

                        //$("#subtitle").show().html("<p>Correct!</p> The word is "+ this.stimuliObj.targets[this.stims[this.n]]).css("color","blue");
                        $("#subtitle").show().html("<p>Correct!</p>").css("color","blue");
                        
                        
                        //The correct answer is highlighed in blue and bolded (the css values can be changed) while both options remain on the screen
                        if (this.stimuliObj.probes2[this.stims[this.n]] === this.stimuliObj.targets[this.stims[this.n]]){
                        $("#subtitle1").show().html(this.stimuliObj.probes1[this.stims[this.n]]);
                        $("#subtitle2").show().html(this.stimuliObj.probes2[this.stims[this.n]]).css({"color":"blue", "font-weight":"bold"});
                        }
                        else{
                        $("#subtitle1").show().html(this.stimuliObj.probes1[this.stims[this.n]]).css({"color":"blue", "font-weight":"bold"});
                        $("#subtitle2").show().html(this.stimuliObj.probes2[this.stims[this.n]]);
                        }
                        
                        console.log("Showing feedback time is "+ Date.now());
                        ...
                        //$("#subtitle").show().html("<p>Wrong!</p>The word is "+ this.stimuliObj.targets[this.stims[this.n]]).css("color","red");
    ...
    endBlock: function() {
        ...
        $("#subtitle").hide();
        $("#subtitle1").hide();
        $("#subtitle1").css({"color":"", "font-weight":""});
        $("#subtitle2").css({"color":"", "font-weight":""});
    ...
    info: function() {
        ...
        return [this.namespace, this.n, this.stims[this.n], _self.stimuliObj.trialtypes[_self.stims[_self.n]], _self.stimuliObj.filenames[_self.stims[_self.n]], _self.stimuliObj.probes1[_self.stims[_self.n]],_self.stimuliObj.probes2[_self.stims[_self.n]],_self.stimuliObj.targets[_self.stims[_self.n]],_self.stimuliObj.correctKeys[_self.stims[_self.n]],].join(); 
